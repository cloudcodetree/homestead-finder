...
export interface SignInSheetProps {
  ...
  /** Optional explainer copy under the title. Defaults to a generic
   *  homestead-finder pitch; Landing overrides with a marketing
   *  variant. */
  blurb?: string;
}

/**
 * Modal sign-in sheet shared by AuthButton (header) and Landing
 * (marketing). Email magic-link is the primary input — works on
 * every device and doesn't depend on third-party cookies, which
 * iOS Safari and most mobile in-app browsers block by default.
 * Google OAuth is the secondary option below the divider.
 *
 * Two state transitions: "sending email" spinner → "check your
 * inbox" confirmation. After magic-link send the modal stays open
 * with the confirmation; users close manually or click the link in
 * email which opens a fresh session in any browser.
 */
export const SignInSheet = ({ onClose, onGoogle, onEmail, blurb }: SignInSheetProps) => {
        ...
        {sentTo ? (
          <div className="mt-5 rounded-lg border border-emerald-200 bg-emerald-50 p-4 text-sm">
            ...
            <p className="text-emerald-800">
              We sent a sign-in link to <strong>{sentTo}</strong>. Click it on
