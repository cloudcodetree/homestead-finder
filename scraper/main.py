...
def run(
    ...
    # data produced by other sources. Strategy: keep every existing listing
    # whose `id` wasn't regenerated by this run, then append/replace with
    # current results. Preserves AI/geo enrichment on unchanged listings.
    output_path = config.DATA_DIR / "listings.json"
    merged: list[dict] = list(scored)
    current_ids = {p.get("id") for p in scored if p.get("id")}
    if output_path.exists():
        try:
            existing = json.loads(output_path.read_text())
            if isinstance(existing, list):
                kept = [p for p in existing if p.get("id") not in current_ids]
                merged = kept + scored
                if kept:
                    print(
                        f"  Merged with existing: {len(kept)} preserved + "
                        f"{len(scored)} from this run = {len(merged)} total"
                    )
